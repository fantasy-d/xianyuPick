...
> 计划状态：渠道配置、runtime 快照、结果追溯与资产展示链路已完成；真实筛选动作闭环承接 `2026-06-26-channel-search-capability-config`
> 当前阶段：阶段 6 回归验证已通过
> 关联计划：
...
- [x] 系统设置页可按渠道配置 11 个筛选项
- [x] 不同渠道间的筛选配置互不污染
- [x] 渠道配置能进入 runtime 快照
- [x] 抓取结果能回写渠道筛选快照
- [x] 决策资产库顶层能展示使用了哪些渠道
- [x] 详情页能按渠道展示，并带出筛选摘要
- [x] 第一批 query 候选项形成真实生效闭环
- [x] 第二批/第三批/第四批项至少有明确降级与后续结论

## 13. 下一步建议

当前计划的配置与追溯链路已经完成。后续新增筛选项时，按以下顺序推进：

1. 在 `src/xianyu_tools/channel_search_filters.py` 增加共享定义。
2. 在配置归一化契约中确认按渠道隔离。
3. 在 runtime 审计中确认 query / UI / combo / special panel 的映射类型。
4. 用 `scripts/validate_source_channel_config.py` 全量回归。
5. 若涉及真实站点动作，把强证据闭环记录到 `plan/2026-06-26-channel-search-capability-config/`。
